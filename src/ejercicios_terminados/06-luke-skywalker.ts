...
(() => {
  ...
  getRequest(`${SW_API}/people/2`).pipe(
    ...
    //La siguiente linea solo sirve para la primera respuesta
    // switchMap(item => getRequest(item.species[0])),

    //Respuesta con mayor dificultad
    switchMap(resp => zip(of(resp), getRequest(resp.species[0]))),
    /* COn zip podemos unir dos observables, es decir, podemos concatenarlos y aso poder extraer toda la información */
